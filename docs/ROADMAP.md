...
| **Full Markdown README rendering** | Partly shipped in v1.7 (`--preview`) | **No stdlib Markdown parser**, so GFM fidelity stays out. The reduced in-house subset renderer this row once dismissed *is* what shipped — opt-in, raw HTML escaped, URL schemes allowlisted (`servery/_markdown.py`), alongside a stdlib-only syntax highlighter (`servery/_highlight.py`). Same precedent as the pure-stdlib QR encoder below: scope it down, write it in-house, document the boundary. |
...
| **Dynamic handlers — CGI / WSGI / ASGI** | Planned (opt-in, off by default) | Host the operator's *own* script/app via the standard interfaces, **all pure stdlib** (`subprocess` / `wsgiref` / `asyncio`), behind explicit `--cgi` / `--wsgi` / `--asgi` flags, gated like `--upload`. servery still defines no routes or middleware (that stays **Never**, below) — it *hosts* an interface, it does not become a framework. Feasibility prototyped (all three work zero-dep); phased plan, 3.14+ design, security model, and per-phase external test harnesses in **[DYNAMIC.md](DYNAMIC.md)**. |
